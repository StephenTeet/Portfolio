![Data Science Portfolio banner](stdsban1.png)

# Hi, I'm Stephen 👋  
### Data Science • GIS • AI • Data Vizualisation

I’m a data‑driven professional with a background in environmental science, GIS, Data Science and Data Vizualisation. I specialize in transforming complex datasets into clear, actionable insights using tools like SQL, Python, R, and ArcGIS.

My work blends analytical thinking with spatial problem‑solving. Whether I’m building Machine Learning models, analyzing geospatial patterns, or cleaning messy data, I focus on uncovering insights that support real‑world decisions.

---

## 🔍 What I Do
- **Data Science:** Python, SQL, R, Excel, Machine Larning, predictive modeling  
- **GIS & Geospatial Analysis:** ArcGIS Pro, QGIS, spatial analysis, geoprocessing  
- **Python for Analytics:** Pandas, data cleaning, automation  
- **Environmental & Geospatial Data:** Raster/vector workflows, mapping, reporting  

---

## 📂 Featured Projects

### 🐍 Python Data Science/ML
- **Restaurant Recommender**  
  Python / Machine Learning project leveraging existing data and tools to help locate the perfect restaurant near you.  
  [Notebook](#) • [Repository](DataScience/RestaurantRecommender/readme.md#)

- **Python Project 2**  
  Automation or ETL workflow for \_\_\_.  
  [Notebook](#) • [Repository](#)

---

### 🗺️ GIS & Spatial Analysis
- **GIS Project 1**  
  Spatial analysis of \_\_\_ using ArcGIS/QGIS.  
  [View Map](#) • [Repository](#)

- **GIS Project 2**  
  Suitability or risk analysis for \_\_\_.  
  [View Map](#) • [Repository](#)

---

### 🧮 SQL Case Studies
- **SQL Case Study 1**  
  Analytical queries answering business questions about \_\_\_.  
  [Repository](#)

- **SQL Case Study 2**  
  Window functions, joins, and CTEs to analyze \_\_\_.  
  [Repository](#)

---

### 🏆 Capstone Projects
- **End‑to‑End Analytics Project**  
  Full pipeline: data collection → cleaning → SQL → visualization.  
  [Repository](#)

- **GIS Automation Project**  
  Automated geoprocessing workflow using ModelBuilder or Python.  
  [Repository](#)

---

## 🛠️ Tools & Technologies
**Analytics:** Power BI, Tableau, Excel, SQL, Python  
**GIS:** ArcGIS Pro, QGIS, StoryMaps  
**Data:** Pandas, APIs, ETL workflows  
**Other:** GitHub, Jupyter, ModelBuilder  

---

## 📫 Contact
- **LinkedIn:** [Stephen Teet](https://www.linkedin.com/in/stephen-teet/)  
- **Email:** [Stephen Teet](teet.stephen@gmail.com)  
- **Portfolio Website (optional):** [Stephen's Portfolio]

Thanks for stopping by — feel free to explore my projects or reach out if you'd like to connect.
